...
void matXvec(const Mat& m, const Vec& v, Vec& ans)
{
   ...
   cudaMalloc(&dev_m.data, m.row() * m.col() * sizeof(float));
   cudaMemcpy(dev_m.data, m.data, m.row()*m.col()*sizeof(float), cudaMemcpyHostToDevice);

    cudaMalloc(&dev_v, v.size() * sizeof(float));
    cudaMemcpy(dev_v.data, v.data, v.size()*sizeof(float), cudaMemcpyHostToDevice);

    cudaMalloc(&dev_ans, ans.size() * sizeof(float));
    ...
    cudaMemcpy(ans.data, dev_ans.data, ans.size() * sizeof(float), cudaMemcpyDeviceToHost);

    cudaFree(dev_m.data);
    cudaFree(dev_v.data);
    cudaFree(dev_ans.data);
